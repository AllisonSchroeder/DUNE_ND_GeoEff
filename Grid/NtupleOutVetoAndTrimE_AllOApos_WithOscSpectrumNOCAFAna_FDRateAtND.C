...
void ProcessFile(TFile *fHad, TFile *fMu){
  ...
  assert(!fHad->IsZombie() && !fMu->IsZombie()); // checks for input files fhad and fmu. if either file is missing, this line should stop execution and give error. 
  
  //Load in Ttrees from files
  TTree* t_effTree = 0;
  ...
  bool scaleToCombinedEfficiency = false; //set to false if hadron eff scaling is desired //DOES NOT CURRENTLY DO ANYTHING...
...
//newlyadded
  ...
  vector<Int_t> *iwritten_vec = 0;            // event numbers
  ...
  Long64_t tentry = t_PosVec->LoadTree(0); //There is also a branch in the t_PosVec tree that just has the combined Off-axis positions... why are we not using that?
  ...
  int nDetPos = ND_LAr_dtctr_pos_vec->size();
  ...
  TFile* FileWithHistoInfo = new TFile("FileWithHistEtrim_MuAndHaddEff_VisEtrim_WithOscProb_FDEvRateAtND_NDFV4m_2DHistosWithSelectedAndThrownEvents_WithCAFLikeMuCut_AllOAPos.root", "RECREATE");
  ...
  float EtrimVal;
  double EmuVal;
  double weightPmuonVal;
  double VisEtrim;
  double oscProb;
  double weightCAFLikeVal;
  double FDEventRateEtrueVal;
  double FDEventRateVal;

  TTree *EtrimTreeEvent = new TTree("EtrimTreeEvent","EtrimTreeEvent");
  EtrimTreeEvent->SetDirectory(FileWithHistoInfo);
  EtrimTreeEvent->Branch("oscProb",&oscProb,"oscProb/D");
  EtrimTreeEvent->Branch("weightCAFLike",&weightCAFLikeVal,"weightCAFLikeVal/D");

  TTree *EtrimTreePos = new TTree("EtrimTreePos","EtrimTreePos");
  EtrimTreePos->SetDirectory(FileWithHistoInfo);
  EtrimTreePos->Branch("OAPos",&OAPos,"OAPos/D");
  EtrimTreePos->Branch("WeightEventsAtOaPos",&WeightEventsAtOaPos,"WeightEventsAtOaPos/D");

  TTree *EtrimTree = new TTree("EtrimTree","Checking weights in Selected Events Histogram");
  EtrimTree->SetDirectory(FileWithHistoInfo);
  EtrimTree->Branch("OAPos",&OAPos,"OAPos/D");
  EtrimTree->Branch("WeightEventsAtOaPos",&WeightEventsAtOaPos,"WeightEventsAtOaPos/D");
  EtrimTree->Branch("Etrim",&EtrimVal,"EtrimVal/F");
  EtrimTree->Branch("VisEtrim",&VisEtrim,"VisEtrim/D");
  EtrimTree->Branch("Emu",&EmuVal,"EmuVal/D");
  EtrimTree->Branch("weightPmuon",&weightPmuonVal,"weightPmuonVal/D");
  EtrimTree->Branch("weightCAFLike",&weightCAFLikeVal,"weightCAFLikeVal/D");
  EtrimTree->Branch("FDEventRateEtrue",&FDEventRateEtrueVal,"FDEventRateEtrueVal/D");
  EtrimTree->Branch("FDEventRate",&FDEventRateVal,"FDEventRateVal/D");
  ...
  { //looping over events to fill the true/gen energy/mom histograms, calculate weightCAFLike each event, and calculate the off-axis pos sampling frequency histogram HistOAPos
    ...
    for (int i_entry = tot_size*i_iwritten; i_entry < tot_size * (i_iwritten+1); i_entry++ ) //i_entry goes from event# * # of off-axis positions (# of dtctr pos * #vtxX pos w/in dtctr) to # of vtxX positon * (ev #+1) effectively a loop over off-axis positions for diff events
    { 
      /*We are looping through the entries in a "flattened" Ttree: t_effValues has a total of #events*#vtxXpositions*#dtctrPos. 
            The first #vtxXpositions*#dtctrPos belong to event 0, the next #vtxXpositions*#dtctrPos belong to event 1, etc. */
      t_effValues->GetEntry(i_entry);

      //cout<<" iev = "<< i_iwritten<<"  ND_LAr_vtx_pos " << ND_LAr_vtx_pos<<" i_entry: "<<i_entry<<endl;

      if(ND_LAr_vtx_pos == -196.45) {//only want to fill the histos for 1 vtxX  ///CALCULATING weightCAFLike COULD ACCIDENTLY BE SKIPPED IF VTX POSITIONS CHANGE AND -196.45 is no longer included as a vertex position
      ...
      if(i_iwritten == 0){
        OAPos2 = ND_LAr_vtx_pos/100.0 + ND_LAr_dtctr_pos/100;
        HistOAPos->Fill(OAPos2);
      }
    }
    if (i_iwritten == 0) HistOAPos->Write("HistOAPos");
  }
  ...
  for (Int_t i_iwritten = 0; i_iwritten<nFDEvents; i_iwritten++) 
  {
    ...
    //Initalize Histograms for Muon Efficiency Plots
    PlotMuonEfficiencyVsVtxX[i_iwritten] = new TGraph(numOApos, vtxX->data(), muon_selected_eff->data());
    PlotMuTrackerEfficiencyVsVtxX[i_iwritten] = new TGraph(numOApos, vtxX->data(), muon_tracker_eff->data());
    PlotCombinedEfficiencyVsVtxX[i_iwritten] = new TGraph(numOApos, vtxX->data(), combined_eff->data());
    Double_t y_geoeff[numOApos];
    ...
    cout<< "      There are " << weightPmuon->size() << " entries in weightPmuon" << endl;
    ...
    for (int i_ND_LAr_vtx_pos = 0; i_ND_LAr_vtx_pos < numOApos; i_ND_LAr_vtx_pos++ )
    {
      ...
      y_geoeff[i_ND_LAr_vtx_pos] = ND_GeoEff;
      
      int skip_empty_weightPmuon = i_ND_LAr_vtx_pos / 48; // Every 48 vertex positions there is an empty weightPmuon value. Use skip_empty_weightPmuon to skip the correct number of entries in weightPmuon
      
      if (ND_LAr_vtx_pos+ND_LAr_dtctr_pos != (*vtxX)[i_ND_LAr_vtx_pos]) {
        cout << "     !!!!!! OA position does NOT match between hadron and muon entries. !!!!!" << endl;
      }
      OAPos = (*vtxX)[i_ND_LAr_vtx_pos]/100;
      ///NOTES TO ALLISON >>> COME UP WITH A WAY TO CHECK THAT THE CORRECT ETRIM AND MUON WEIGHT ARE BEING ASSOCIATED WITH EACH OTHER>>>

      // Use these lines to check looping if needed. 
      //cout << "   NumThrowsCounter = " << NumThrowsCounter << endl;
      //cout << "   NPassedThrows = " << NPassedThrows << endl;

      WeightEventsAtOaPos = HistOAPos->GetBinContent(HistOAPos->FindBin(OAPos));
     ...
     EtrimTreePos->Fill();
      ...
      for (Int_t ithrow = NumThrowsCounter; ithrow < NPassedThrows+NumThrowsCounter; ithrow++ ){ 
        //NPassedThrows is the number of throws that passed the hadron containment cut at this vertex, and we have to skip the previous NumThrowsCounter throws to get the correct weightPmuon value
	      ThrowInfo info;
        ...
        if ((*weightPmuon)[ithrow + skip_empty_weightPmuon +1].empty()) { // Every 48 vertex positions there is an empty weightPmuon value. Use skip_empty_weightPmuon to skip the correct number of entries in weightPmuon
          cout << "WARNING: weightPmuon row " << (ithrow + skip_empty_weightPmuon + 1) << " is empty. Skipping throw." << endl; //The first entry in weightPmuon is empty, so we have to skip 1 entry to get to the correct one, hence the ithrow+1
          cout << "   Vertex index = " << i_ND_LAr_vtx_pos << endl;
          cout << "   Vertex position within detector: " << ND_LAr_vtx_pos << endl;
          cout << "   Detector position: " << ND_LAr_dtctr_pos << endl;
          continue; 
        } else {
          info.weightPmuon = (*weightPmuon)[ithrow +skip_empty_weightPmuon +1][0];
          if (info.weightPmuon > 1) {
            cout << "weightPmuon = " << info.weightPmuon << " for OApos = " << OAPos << endl;
          }
        }
        ...
        /*if (info.weightPmuon* 1.0/WeightEventsAtOaPos  * weightCAFLike[i_iwritten] * FDEventRateAtND_ETrue(cacheEtrue, EnuTrue[i_iwritten], OAPos) > 1) {
            cout << "Warning: Weights are greater than 1 for OApos = " << OAPos << endl; 
            cout << "   info.weightPmuon =                                                " << info.weightPmuon << endl;
            cout << "   1.0/WeightEventsAtOaPos =                                         " << 1.0/WeightEventsAtOaPos << endl;
            cout << "   FDEventRateAtND_ETrue(cacheEtrue, EnuTrue[i_iwritten], OAPos) = " << FDEventRateAtND_ETrue(cacheEtrue, EnuTrue[i_iwritten], OAPos) << endl;
        }*/

        VisEtrim = (info.Etrim + info.Emu)/1000;
        weightPmuonVal = info.weightPmuon;
        EtrimVal = info.Etrim;
        EmuVal = info.Emu;
        weightCAFLikeVal = weightCAFLike[i_iwritten];
        FDEventRateEtrueVal = FDEventRateAtND_ETrue(cacheEtrue, EnuTrue[i_iwritten], OAPos);
        FDEventRateVal = FDEventRateAtND(cacheLepHad, info.Etrim *1E-3 , info.Emu*1E-3, OAPos);
        EtrimTree->Fill();
      ...
      NumThrowsCounter += NPassedThrows; 

    }// end vtx loop

    oscProb = calc->P(14,14,EnuTrue[i_iwritten]);
    EtrimTreeEvent->Fill();
    ...
    PlotEfficiencyVsVtxX[i_iwritten] = new TGraph(numOApos, vtxX->data(), y_geoeff);
    
    //Set Efficiency plot titles
  ...
  // Fill Histograms;
  //for (Int_t i_iwritten = 0; i_iwritten<nFDEvents; i_iwritten++) //loop over FD events
  //{
    

    //Do I need these with the modified structure?
   // Double_t x_ND_LAr_vtx_pos[ND_vtx_vx_vec_size];
    ...
    //AllThrowInfo[i_iwritten].resize(numOApos);
    //for (int i_ND_LAr_vtx_pos = 0; i_ND_LAr_vtx_pos < numOApos; i_ND_LAr_vtx_pos++ ) //loop over off-axis position
    //{
      /*because of how t_effValues is structured, we need to skip the previous events' entries in t_effValues 
        (there are numOApos*i_iwritten previous entries to skip), so we use hadEntry to do so. */
      //Int_t hadEntry = numOApos*i_iwritten+i_ND_LAr_vtx_pos; 
      //t_effValues->GetEntry(hadEntry);
      //cout << "Where are we crashing? 1" << endl;
      ...
      //cout << "ND_LAr_vtx_pos: " << ND_LAr_vtx_pos << ", ND_LAr_dtctr_pos: " << ND_LAr_dtctr_pos << endl;
      //OAPos = ND_LAr_vtx_pos/100.0 + ND_LAr_dtctr_pos/100;
      ...
      //cout << "Where are we crashing? 2" << endl;
      ...
      //for (const auto& info : throwList) {
        ...
        //cout << "Where are we crashing? Throw loop" << endl;
        //SelectedEventsVsOAPosVsTotalETrim[i_iwritten]->Fill((info.Etrim + info.Emu)/1000 ,OAPos, info.weightPmuon* 1.0/WeightEventsAtOaPos  * weightCAFLike[i_iwritten] * FDEventRateAtND_ETrue(cacheEtrue, EnuTrue[i_iwritten], OAPos)); //FDEventRateAtND(cacheLepHad, info.Etrim *1E-3 , info.Emu*1E-3, OAPos));
        //AllThrownEventsVsOAPosVsTotalETrim[i_iwritten]->Fill((info.Etrim + info.Emu)/1000 , OAPos, double(validThrows)/throwList.size()* 1.0/WeightEventsAtOaPos * FDEventRateAtND_ETrue(cacheEtrue, EnuTrue[i_iwritten], OAPos)); //FDEventRateAtND(cacheLepHad, info.Etrim *1E-3 , info.Emu*1E-3, OAPos));
    ...
    //cout << "Where are we crashing? Post Scaling 4" << endl;
