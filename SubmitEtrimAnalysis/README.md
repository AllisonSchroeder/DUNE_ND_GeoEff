...
3.0 Create working directory
```bash
mkdir SubmitEtrimAnalysisJobs
cd SubmitEtrimAnalysisJobs
```

3.1 First get the work env setup
...
3.2 Write the list of input root files (from hadron and from muon geo eff) to a txt file each
...
-- this it also changes pnfs to xrootd so that worker node can access
